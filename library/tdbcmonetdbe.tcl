...
# tdbcmonetdbe.tcl --
#
#      Tcl DataBase Connectivity MonetDB/e Driver
#      Class definitions and Tcl-level methods for the tdbc::monetdbe bridge.
#
#------------------------------------------------------------------------------
...
package provide tdbc::monetdbe 0.2.0


::namespace eval ::tdbc::monetdbe {
...
#------------------------------------------------------------------------------
#
# tdbc::monetdbe::connection --
#
#	Class representing a connection to a MonetDB/e database.
#
#-------------------------------------------------------------------------------

::oo::class create ::tdbc::monetdbe::connection {
    ...
    constructor {url args} {
        ...
        if {[catch {monetdbe [namespace current]::DB -url $url} errorMsg]} {
            error $errorMsg
        }
    ...
    forward statementCreate ::tdbc::monetdbe::statement create

    # No option to config, return error.
    ...
    method primarykeys {table} {
        ...
        set count [$stmt get_row_count]
        for {set i 0} { $i < $count} {incr i 1} {
            set rowlist [$stmt fetch_row_list]
            set tablename [lindex $rowlist 0]
            set schemaname [lindex $rowlist 1]
            set keyname [lindex $rowlist 2]
...
#------------------------------------------------------------------------------
#
# tdbc::monetdbe::statement --
#
#	The class 'tdbc::monetdbe::statement' models one statement against a
#       database accessed through a MonetDB/e connection
#
#------------------------------------------------------------------------------

::oo::class create ::tdbc::monetdbe::statement {
    ...
    forward resultSetCreate ::tdbc::monetdbe::resultset create
    ...
    method paramtype args {
        ...
        if {$length > $count} {
            ...
            if {$type in {bool tinyint smallint integer bigint string \
                          float double date time timestamp blob}} {
...
#------------------------------------------------------------------------------
#
# tdbc::monetdbe::resultset --
#
#	The class 'tdbc::monetdbe::resultset' models the result set that is
#	produced by executing a statement against a MonetDB/e database.
#
#------------------------------------------------------------------------------

::oo::class create ::tdbc::monetdbe::resultset {
